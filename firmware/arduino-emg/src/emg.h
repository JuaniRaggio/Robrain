...
private:
  ...
  struct ChannelReader {
    ...
    template <uint64_t N> uint8_t get_copy(uint8_t (&out)[N]) const;
    template <uint64_t N> uint8_t get_copy(uint8_t n, uint8_t (&out)[N]) const;
    bool is_full();
  };
...
public:
  ...
  // bytes available
  template <uint64_t N>
  uint8_t get_data(Muscle muscle, uint8_t (&out)[N]) const;
