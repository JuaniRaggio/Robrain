...
void init(motor::WheelPair &pair) {
    ...
    // PC → ESP32
    NimBLECharacteristic *pCmdChar = pService->createCharacteristic(CMD_CHAR_UUID, NIMBLE_PROPERTY::WRITE);
    pCmdChar->setCallbacks(new CmdCharCallbacks());

    // ESP32 → PC
    pService->createCharacteristic(STATUS_CHAR_UUID, NIMBLE_PROPERTY::NOTIFY | NIMBLE_PROPERTY::READ);
    pService->start();
...
void set_connect_callback(ConnectCallback cb) { connect_cb = cb; }
